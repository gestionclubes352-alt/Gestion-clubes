...
import type { RivalPlayer } from '../types';

interface RivalBulkPhotoUploadProps {
  players: RivalPlayer[];
  clubId: string;
  onClose: () => void;
  onUploaded: (playerId: RivalPlayer['id'], fotoUrl: string) => Promise<void>;
}
...
const matchPlayer = (fileName: string, players: RivalPlayer[]): RivalPlayer | undefined => {
  const base = fileName.replace(/\.[^.]+$/, '');
  const norm = normalize(base);
  const tokens = norm.split(' ').filter(Boolean);

  const dorsalMatch = players.find(p => p.dorsal != null && tokens.includes(String(p.dorsal)));
  if (dorsalMatch) return dorsalMatch;

  let best: { player: RivalPlayer; score: number } | undefined;
  players.forEach(p => {
...
const RivalBulkPhotoUpload: React.FC<RivalBulkPhotoUploadProps> = ({ players, clubId, onClose, onUploaded }) => {
  ...
  const sortedPlayers = useMemo(
    () => [...players].sort((a, b) => a.nombre.localeCompare(b.nombre, 'es')),
    [players]
  );
  ...
  const addFiles = (fileList: FileList | File[]) => {
    ...
    const newRows: Row[] = files.map(file => {
      const matched = matchPlayer(file.name, players);
      return {
  ...
  const handleUploadAll = async () => {
    ...
    for (const row of rows) {
      ...
      try {
        const fotoUrl = await uploadRivalPlayerPhoto(row.file, row.playerId, clubId);
...
export default RivalBulkPhotoUpload;
